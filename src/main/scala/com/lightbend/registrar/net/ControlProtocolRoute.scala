...
object ControlProtocolRoute {
  ...
  def apply(registrationHandlerRef: ActorRef[Message])(implicit ec: ExecutionContext, settings: Settings, scheduler: Scheduler) = {
    ...
    path("ping") {
      ...
      pathPrefix("topics") {
        (get & pathEnd) {
          ...
          (post & path(Segment / "register") & entity(as[String])) {
            case (topic, name) =>
